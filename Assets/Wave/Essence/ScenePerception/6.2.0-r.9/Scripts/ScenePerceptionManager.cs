...
namespace Wave.Essence.ScenePerception
{
	...
	public class ScenePerceptionManager : MonoBehaviour
	{
		...
		{
			return CacheSpatialAnchor(new WVR_SpatialAnchorName(cachedSpatialAnchorName), anchorHandle);
		}
		...
		{
			return UncacheSpatialAnchor(new WVR_SpatialAnchorName(cachedSpatialAnchorName));
		}
				...
				if (logCached) {
					...
					if (cachedSpatialAnchorNames != null && cachedSpatialAnchorNames.Length > 0)
						foreach (var name in cachedSpatialAnchorNames)
							Log.v(TAG, TSCSB.Append("  ").Append(name.ToString()));
				}
			}
		...
		public WVR_Result GetCachedSpatialAnchorNames(out string[] cachedSpatialAnchorNames)
		{
			...
			for (int i = 0; i < cachedWVRSpatialAnchorNames.Length; i++)
				cachedSpatialAnchorNames[i] = cachedWVRSpatialAnchorNames[i].ToString();
			return result;
		}
		...
		public WVR_Result CreateSpatialAnchorFromCacheName(string cachedAnchorName, string spatialAnchorName, out UInt64 anchor)
		{
			...
			info.spatialAnchorName = new WVR_SpatialAnchorName(spatialAnchorName);
			info.cachedSpatialAnchorName = new WVR_SpatialAnchorName(cachedAnchorName);
		...
		{
			return PersistSpatialAnchor(new WVR_SpatialAnchorName(persistedSpatialAnchorName), anchor);
		}

		public WVR_Result UnpersistSpatialAnchor(string name)
		{
			var persistedSpatialAnchorName = new WVR_SpatialAnchorName(name);
			WVR_Result result;
			lock (lockObject)
			...
			else
			{
				if (logPersisted)
				{
					...
					if (persistedSpatialAnchorNames != null && persistedSpatialAnchorNames.Length > 0)
						...
						{
							sb.AppendLine().Append("  ").Append(name.ToString());
						}
					Log.v(TAG, sb);
				}
			}
		...
		public WVR_Result GetPersistedSpatialAnchorNames(out string[] persistedSpatialAnchorNames)
		{
			...
			for (int i = 0; i < persistedSpatialAnchorWVRNames.Length; i++)
				persistedSpatialAnchorNames[i] = persistedSpatialAnchorWVRNames[i].ToString();
			return result;
		}
			...
			out UInt64 anchor /* WVR_SpatialAnchor* */)
		{
			...
			createInfo.persistedSpatialAnchorName = new WVR_SpatialAnchorName(persistedSpatialAnchorName);
			createInfo.spatialAnchorName = new WVR_SpatialAnchorName(spatialAnchorName);
			WVR_Result result;
			lock (lockObject)
			...
			out byte[] data)
		{
			...
			var persistedSpatialAnchorWVRName = new WVR_SpatialAnchorName(persistedSpatialAnchorName);
			WVR_Result result;
			lock (lockObject)
			{
		...
		public WVR_Result GetSpatialAnchorState(UInt64 anchorHandle, WVR_PoseOriginModel originModel, out SpatialAnchorTrackingState anchorTrackingState, out Pose pose, out string anchorName)
		{
			...
			anchorName = anchorState.anchorName.ToString();
